...
| レイヤー | 技術 | バージョン | 説明 |
|---------|------|-----------|------|
| **バックエンド** | Laravel | 12.x | PHP Webフレームワーク |
...
| **認証** | Laravel Sessions + Socialite | - | セッションベース認証 + OAuth |
| **OAuth** | Laravel Socialite | 5.x | Google/Discord認証 |
| **ホスティング** | Google Cloud Run | - | コンテナホスティング |
...
Laravel標準のセッションベース認証と、Laravel Socialiteを使用したOAuth認証を採用しています。
...
### OAuth認証（Google/Discord）
1. ユーザーが「Googleでログイン」または「Discordでログイン」をクリック
2. 各プロバイダーの認証画面へリダイレクト
3. ユーザーがアプリを承認
4. コールバックURLに戻り、以下の処理を実行:
   - プロバイダーIDで既存ユーザーを検索 → ログイン
   - メールアドレスで既存ユーザーを検索 → アカウント連携してログイン
   - 該当なし → 新規ユーザー作成してログイン
5. 対戦記録画面へリダイレクト
...
| `OAuthController` | Google/Discord OAuth認証 |
...
### 認証・認可
- **セッション管理**: 環境変数で切り替え（開発: `database`, Cloud Run: `cookie`）
