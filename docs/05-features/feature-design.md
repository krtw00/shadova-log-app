...
| フィードバック | バグ報告・機能要望・お問い合わせ | 認証ユーザー |
...
|----------|------|-----------|
| 背景透過 | 透過背景の有効/無効 | 有効 |
| フォントサイズ | small/medium/large/xlarge | medium |
| カラーテーマ | dark/light/custom | dark |
| カスタム背景色 | カスタム時の背景色（HEX） | - |
| カスタム文字色 | カスタム時の文字色（HEX） | - |
| 勝率表示 | 表示/非表示 | 表示 |
| 戦績表示 | 表示/非表示 | 表示 |
| 連勝表示 | 表示/非表示 | 表示 |
| デッキ情報表示 | 使用中デッキの統計を表示 | 表示 |
| 対戦ログ表示 | 直近の対戦履歴を表示 | 表示 |
| 対戦ログ件数 | 表示する対戦ログの件数 | 5 |
...
ユーザーからのバグ報告・機能要望・お問い合わせをGitHub Issueと連携して管理する機能。
...
送信されたフィードバックは、GitHub APIを通じてリポジトリにIssueとして自動作成されます。

**ラベル付与:**
- バグ報告: `bug`, `user-reported`
- 機能要望: `enhancement`, `user-reported`
- お問い合わせ: `question`, `user-reported`

**Issue タイトル形式:**
- バグ報告: `[Bug] {タイトル}`
- 機能要望: `[Feature Request] {タイトル}`
- お問い合わせ: `[Question] {件名}`
...
1. バグ報告
   ...
   1. フィードバック画面にアクセス
   2. 「バグ報告」タブを選択
   3. タイトルと説明を入力（再現手順等はオプション）
   4. 送信ボタンをクリック
   5. GitHub Issueが自動作成される
   6. 成功メッセージが表示される
...
| フィードバック種別 | フィールド | ルール |
|-------------------|-----------|--------|
| バグ報告 | title | 必須、最大255文字 |
| バグ報告 | description | 必須、最大5000文字 |
| バグ報告 | steps | 任意、最大3000文字 |
| バグ報告 | expected | 任意、最大1000文字 |
| バグ報告 | actual | 任意、最大1000文字 |
| 機能要望 | title | 必須、最大255文字 |
| 機能要望 | description | 必須、最大5000文字 |
| 機能要望 | use_case | 任意、最大2000文字 |
| お問い合わせ | subject | 必須、最大255文字 |
| お問い合わせ | message | 必須、最大5000文字 |
